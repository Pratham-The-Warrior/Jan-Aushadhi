// ============================================================
// Seller Central — App Router  (v2 — Polished UI)
...
import {
  LayoutDashboard, ShoppingCart, Package, Store, BarChart3, LogOut, Menu, X, Bell, Sparkles
} from 'lucide-react';
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: 'var(--color-surface-900)' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
          <span style={{ color: 'var(--color-text-primary)', fontSize: '0.9375rem', fontWeight: 600 }}>Seller Central</span>
          <span style={{ color: 'var(--color-text-muted)', fontSize: '0.8125rem' }}>Preparing your dashboard...</span>
        </div>
  ...
  return (
    <>
      ...
      <aside className={`sidebar ${mobileOpen ? 'flex!' : ''}`} style={mobileOpen ? { display: 'flex' } : undefined}>
        ...
        {/* Store Info */}
        {sellerProfile && (
          <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--color-border)', background: 'rgba(16, 185, 129, 0.03)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'var(--color-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {sellerProfile.name}
                </p>
                <p style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)', marginTop: 1, fontFamily: 'monospace' }}>
                  {sellerProfile.pmbjk_code}
                </p>
              </div>
            </div>
          </div>
        ...
        <div>
          ...
          {sellerProfile && (
            <p style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)', marginTop: 1 }}>
              {getGreeting()}, {sellerProfile.name?.split(' ')[0] || 'Operator'}
            </p>
          )}
        </div>
      </div>
